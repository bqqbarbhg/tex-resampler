...
void sdt_dead_reckoning(unsigned int width, unsigned int height, unsigned char threshold, const unsigned char* image, unsigned int stride, float* distance_field, int* position_field);
...
void sdt_dead_reckoning(unsigned int width, unsigned int height, unsigned char threshold, const unsigned char* image, unsigned int stride, float* distance_field, int* position_field) {
	...
	#define I(x, y) (image[((x) + (y) * width) * stride] > threshold)
	...
	if (position_field) {
		for(unsigned int y = 0; y < height; y++) {
			for(unsigned int x = 0; x < width; x++) {
				position_field[x + y*width] = (PX(x+1, y+1)-1) + (PY(x+1, y+1)-1) * width;
			}
		}
	}
